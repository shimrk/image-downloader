...
const fileNamingSelect = document.getElementById('fileNaming') as HTMLSelectElement;
const autoDownloadCheckbox = document.getElementById('autoDownload') as HTMLInputElement;
...
function validateSettings(settings: Settings): boolean {
    // ダウンロードパスのバリデーション
    if (settings.downloadPath.includes('..') || settings.downloadPath.includes('\\')) {
